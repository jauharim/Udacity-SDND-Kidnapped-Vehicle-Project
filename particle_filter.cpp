...
void getinrangelandmarks(Particle p, double sensor_range, Map map_landmarks, vector<LandmarkObs> in_range_landmarks)
{
    //particle position
    double x1 = p.x;
    double y1 = p.y;
    ...
    for (unsigned int i = 0;i < map_landmarks.landmark_list.size();i++)
    {
        ...
        if (dist <= sensor_range)
        {
            ...
            //add landmark to in range observations
            in_range_landmarks.push_back(obs);
        }
    }
}

void tobs(Particle p, vector<LandmarkObs>& transformed_observations)
{
    double xt = p.x;
    double yt = p.y;
